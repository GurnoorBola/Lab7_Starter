describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const button = await shadowRoot.$('button');
      const innerTextValue = await (await button.getProperty('innerText')).jsonValue();
      if (innerTextValue === 'Add to Cart') {
        await button.click();
      }
    }
    const cartCount = await page.$('#cart-count');
    const innerTextValue = await (await cartCount.getProperty('innerText')).jsonValue();
    expect(innerTextValue).toBe('20');
  }, 20000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const button = await shadowRoot.$('button');
      const innerTextValue = await (await button.getProperty('innerText')).jsonValue();
      expect(innerTextValue).toBe('Add to Cart');
    }
    const cartCount = await page.$('#cart-count');
    const innerTextValue = await (await cartCount.getProperty('innerText')).jsonValue();
    expect(innerTextValue).toBe('0');
